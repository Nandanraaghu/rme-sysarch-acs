...
UINT64 pal_get_xsdt_ptr(VOID);
UINT64 pal_get_fadt_ptr(VOID);

#define PAL_ACPI_MAX_ROOT_BRIDGES 32u
...
#define PCIE_EXTRACT_BDF_SEG(bdf)  (((bdf) >> 24) & 0xFFu)
#define PCIE_EXTRACT_BDF_BUS(bdf)  (((bdf) >> 16) & 0xFFu)
...
typedef struct {
  UINT32 end_offset;
  PAL_ACPI_ROOT_BRIDGE_INFO info;
} PAL_AML_DEVICE_SCOPE;

STATIC PAL_ACPI_ROOT_BRIDGE_INFO g_root_bridges[PAL_ACPI_MAX_ROOT_BRIDGES];
...
pal_acpi_parse_aml(CONST UINT8 *aml, UINT32 length)
{
  ...
  PAL_AML_DEVICE_SCOPE stack[PAL_AML_MAX_DEVICE_DEPTH];
  INT32 depth = -1;
  UINT32 offset = 0u;
  ...
  while (offset < length) {
    while ((depth >= 0) && (offset >= stack[depth].end_offset)) {
      if ((stack[depth].info.hid_is_pci != 0u) &&
          ...
          (g_root_bridge_count < PAL_ACPI_MAX_ROOT_BRIDGES)) {
...
pal_acpi_parse_root_bridges(VOID)
{
  ...
  {
    rme_print(ACS_PRINT_WARN, L" DSDT not found; root bridge AML unavailable ");
    dsdt = NULL;
  }
...
pal_get_madt_ptr()
{
  ...
  Xsdt = (EFI_ACPI_DESCRIPTION_HEADER *) pal_get_xsdt_ptr();
  if (Xsdt == NULL) {
      rme_print(ACS_PRINT_ERR, L" XSDT not found ");
      return 0;
  }
...
/**
  @brief  Iterate through the tables pointed by XSDT and return CEDT address

  @param  None

  @return 64-bit CEDT address
**/
UINT64
pal_get_cedt_ptr()
{
  ...
  Entry64Num = (Xsdt->Length - sizeof(EFI_ACPI_DESCRIPTION_HEADER)) >> 3;
  for (Idx = 0; Idx < Entry64Num; Idx++) {
    if (*(UINT32 *)(UINTN)(Entry64[Idx]) == EFI_ACPI_6_4_CXL_EARLY_DISCOVERY_TABLE_SIGNATURE) {
        return(UINT64)(Entry64[Idx]);
    }
  }
...
{
  if (uid == NULL) {
    rme_print(ACS_PRINT_ERR, L" pal_acpi_get_root_bridge_uid UID pointer NULL ");
    return 1u;
  }

  pal_acpi_parse_root_bridges();
  if (g_root_bridge_count == 0u) {
    if ((segment == 0u) && (bbn == 0u)) {
      *uid = 0u;
      return 0u;
    }
    return 1u;

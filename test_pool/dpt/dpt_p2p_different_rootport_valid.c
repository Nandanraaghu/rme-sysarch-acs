...
#define TEST_NAME  "dpt_p2p_different_rootport_valid"
#define TEST_DESC  "TDI DPT Check - P2P Different RP - Valid Access        "
...
free_mem:
      ...
      {
          val_print(ACS_PRINT_ERR,
                    " free_mem: Failed to add MMU entry for tgt_bar_base: 0x%llx",
                    (uint64_t)tgt_bar_base);
          test_fail++;
          continue;
      }

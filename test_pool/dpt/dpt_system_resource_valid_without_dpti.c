...
#define TEST_NAME "dpt_system_resource_valid_without_dpti"
#define TEST_DESC  "TDI DPT Check - System Resource - Valid Access - No Inv"
...
payload(void)
{
  ...
  for (instance = 0; instance < num_exercisers; ++instance)
  {
      ...
      {
          val_print(ACS_PRINT_ERR,
            " Failed to add MMU entry for bar_base: 0x%lx", bar_base);
          test_fail++;
          continue;
      }
      ...
      if (val_dpt_add_entry((uint64_t)dram_buf_in_virt,

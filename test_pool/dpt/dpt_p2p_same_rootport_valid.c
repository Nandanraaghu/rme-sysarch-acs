...
#define TEST_NAME "dpt_p2p_same_rootport_valid"
#define TEST_DESC  "TDI DPT Check - P2P Same RP - Valid Access             "
...
static
uint32_t
get_target_exer_bdf(uint32_t bdf, uint32_t req_rp_bdf, uint32_t *tgt_e_bdf,
                    uint32_t *tgt_rp_bdf, uint64_t *bar_base, uint32_t *tgt_instance)
{
  ...
  while (instance-- != 0)
  {
      ...
      if (req_rp_bdf == erp_bdf && bdf != e_bdf)
      {
...
payload(void)
{
  ...
  for (instance = 0; instance < num_exercisers; ++instance)
  {
      ...
      if (get_target_exer_bdf(bdf, rp_bdf, &tgt_e_bdf, &tgt_rp_bdf, &tgt_bar_base, &tgt_instance))
      {
          val_print(ACS_PRINT_ERR, " No Exerciser found on the same RP, BDF 0x%x", bdf);
          continue;
      }
      ...
      {
          val_print(ACS_PRINT_ERR, " Failed to set memory to 0x%x for ", TEST_DATA);
          val_print(ACS_PRINT_ERR, "0x%4x ", instance);
          test_fail++;
          continue;
      }
      val_pe_cache_clean_invalidate_range((uint64_t)dram_buf_in_virt, (uint64_t)(dma_len));
      ...
      {
        val_print(ACS_PRINT_ERR,
         " DPT entry was set to RDRW Access, but access was not granted for %4x", instance);
        test_fail++;
        goto free_mem;
      }
...
free_mem:
      ...
      {
          val_print(ACS_PRINT_ERR,
            " free_mem: Failed to add MMU entry for tgt_bar_base: 0x%llx", (uint64_t)tgt_bar_base);
          test_fail++;
          continue;
      }
